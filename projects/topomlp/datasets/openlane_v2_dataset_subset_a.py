...
class OpenLaneV2SubsetADataset(Custom3DDataset):
    ...
    def load_annotations(self, ann_file):
        ...
        if self.yolov8_file is not None:
            f = open(self.yolov8_file, 'rb')
            self.yolov8_det = pickle.load(f)
        ...
        data_infos = self.collection.keys
    ...
    def get_data_info(self, index):
        ...
        if self.yolov8_file is not None:
            te_dets = self.yolov8_det['results'][self.data_infos[index]]['predictions']['traffic_element']
            te_yolov8 = np.zeros([len(te_dets), 6])
            for i, te_det in enumerate(te_dets):
                te_yolov8[i, :4] = te_det['points'].flatten()
                te_yolov8[i, 4] = te_det['attribute']
                te_yolov8[i, 5] = te_det['confidence']
        else:
            te_yolov8 = None
        ...
        if 'train' in self.ann_file or 'val' in self.ann_file:
    ...
    def format_preds(self, results):
        ...
        for index, result in enumerate(results):
            ...
            sorted_index = np.argsort(pred_lc[1][:, 0])[:300]
            ...
            if lanes.shape[1] == 3 or lanes.shape[1] == 4 or lanes.shape[1] == 5:
    ...
    def visualize(self, pred_dict, visualization_dir, visualization_num, confidence_threshold=0.3, **kwargs):
        ...
        for index, (key, prediction) in enumerate(pred_dict['results'].items()):
            ...
            pred_lc_conf = np.array([lc['confidence'] for lc in prediction['lane_centerline']])[pred_lc_mask]
    ...
    for idx, (image, lidar2img) in enumerate(zip(images, lidar2imgs)):
        ...
        if pred_lc is not None:
            for lc in pred_lc:
                ...
                points_2d = points_2d.astype(int)
                image = cv2.polylines(image, points_2d[None], False, COLOR_PRED, 5)
        ...
        for lc_idx, lcte in enumerate(gt_topology_lcte):
            for te_idx, connected in enumerate(lcte):
                if connected:
                    ...
                    image = cv2.arrowedLine(image, (p2[0], p2[1]), (p1[0], p1[1]), COLOR_GT_TOPOLOGY, thickness=3,
                                            tipLength=0.03)
        ...
        for lc_idx, lcte in enumerate(pred_topology_lcte):
            for te_idx, connected in enumerate(lcte):
                if connected:
                    ...
                    image = cv2.arrowedLine(image, (p2[0], p2[1]), (p1[0], p1[1]), COLOR_PRED_TOPOLOGY, thickness=3,
                                            tipLength=0.03)
...
def render_bev(gt_lc=None, pred_lc=None, gt_topology_lclc=None, pred_topology_lclc=None, map_size=[-52, 52, -27, 27],
    ...
    image = np.ones((int(scale * (map_size[1] - map_size[0])), int(scale * (map_size[3] - map_size[2])), 3),
                    dtype=np.uint8) * 255

    if gt_lc is not None:
        for lc in gt_lc:
            draw_coor = (scale * (-lc[:, :2] + np.array([map_size[1], map_size[3]]))).astype(np.int)
            image = cv2.polylines(image, [draw_coor[:, [1, 0]]], False, COLOR_GT, max(round(scale * 0.2), 5))
    ...
    if pred_lc is not None:
        if confidence is None:
            for lc in pred_lc:
                draw_coor = (scale * (-lc[:, :2] + np.array([map_size[1], map_size[3]]))).astype(np.int)
                image = cv2.polylines(image, [draw_coor[:, [1, 0]]], False, COLOR_PRED, max(round(scale * 0.2), 5))
            ...
            for i, lc in enumerate(pred_lc):
                ...
                image = cv2.circle(image, (draw_coor[0, 1], draw_coor[0, 0]), max(round(scale * 0.5), 3), COLOR_PRED,
                                   -1)
                # image = cv2.putText(image, str(confidence[i]), (draw_coor[0, 1], draw_coor[0, 0]), cv2.FONT_HERSHEY_SIMPLEX, 0.75, COLOR_PRED, 2)
