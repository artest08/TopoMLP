...
class TopoLTHead(nn.Module):
    ...
    def loss(self, rel_preds, o1_assign_results, o2_assign_results, gt_adjs):
        ...
        if xs.size(0) != xs.size(1):
            ...
            rel_preds = rel_preds[0][xs_new].view(-1, 1)
        else:
            targets = 1 - targets[0][xs_new][:, ys_new].view(-1).long()
            rel_preds = rel_preds[0][xs_new][:, ys_new].view(-1, 1)

        # if len(rel_preds) == 0:
        #     loss_rel = (rel_preds * 0.0).mean()
        # else:
        loss_rel = self.loss_rel(rel_preds, targets)
        # if xs.size(0) != xs.size(1):
        #     print('gt_clte******{}'.format(gt_adj))
        #     print('lcte*********{}'.format(loss_rel))
        #     if torch.isnan(loss_rel):
        #         print('lcte-------{}'.format(rel_preds.size()))
        # else:
        #     print('lclc*********{}'.format(loss_rel))
